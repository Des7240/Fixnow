...
sequenceDiagram
    ...
    Worker->>Frontend: Update status

    Frontend->>BookingAPI: PATCH /bookings/status

    BookingAPI->>TimelineService: Create timeline event

    TimelineService->>Database: Save booking event

    Database-->>TimelineService: Saved

    TimelineService-->>BookingAPI: Success

...
/// VNPay 2.1.0 spec: signature is computed on URL-encoded key=value pairs joined by '&'.
/// </summary>
public class VNPayProvider : IPaymentProvider
{
  ...
  {
    var tmnCode = (_config["VNPay:TmnCode"] ?? "4YUP19I4").Trim('"', ' ');
    var hashSecret = (_config["VNPay:HashSecret"] ?? "MDUIFDCRAKLNBPOFIAFNEKFRNMFBYEPX").Trim('"', ' ');
    var baseUrl = (_config["VNPay:BaseUrl"] ?? "https://sandbox.vnpayment.vn/paymentv2/vpcpay.html").Trim('"', ' ');
    ...
    vnpay.AddRequestData("vnp_IpAddr", string.IsNullOrEmpty(request.IpAddress) ? "127.0.0.1" : request.IpAddress.Split(',')[0].Trim());
    vnpay.AddRequestData("vnp_Locale", "vn");
    vnpay.AddRequestData("vnp_OrderInfo", (request.Description ?? "Payment").Trim());
  ...
  public Task<PaymentResultDto> VerifyCallbackAsync(IQueryCollection query)
  {
    ...
    {
      if (!string.IsNullOrEmpty(k) && k.StartsWith("vnp_") && !k.Equals("vnp_SecureHash", StringComparison.OrdinalIgnoreCase))
      {
  ...
  private class VnPayLibrary
  {
    ...
    /// Both query string and signature input use URL-encoded values.
    /// </summary>
    ...
    /// Signature input must use URL-encoded values to match the request side.
    /// </summary>
